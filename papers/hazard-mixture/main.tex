...
\item[(e)] \textbf{Relation to the published statements} (Section~\ref{sec:relation}). The extension asked for in Remark~6.3 of \cite{ShojaeeAsadiFinkelstein2022} has a negative answer for every $n\ge 3$. Theorem~3.8 of the accepted manuscript of \cite{SahooKayalFinkelstein2026}, and with it Corollary~3.2 and Theorem~3.9 there, are contradicted by explicit instances; we point to the step of the published proof that does not apply. The same instances contradict the reversed hazard rate statements Theorem~3.11, Corollary~3.3 and Theorem~3.12 there (Section~\ref{sec:rh}). A companion exact-arithmetic audit of both manuscripts (Section~\ref{sec:repro}) certifies further failures in \cite{SahooKayalFinkelstein2026}: the complementary $\mathcal W_n$, $\alpha\le0$ halves of Theorems~3.8 and~3.11 (so both halves of both statements fail), the stochastic-order statement Theorem~3.4 for $\alpha>0$, and Theorem~3.10 under the literal reading of the model; it also shows that the counterexamples printed there as Counterexamples~3.7 and~3.8 are themselves invalid, and records minor errata in \cite{ShojaeeAsadiFinkelstein2022}. The usual stochastic order of the mixtures studied here (Theorem~6.1 of \cite{ShojaeeAsadiFinkelstein2022}), the two-component statements (Theorem~6.3 there and Theorem~3.7 of \cite{SahooKayalFinkelstein2026}, and Theorem~3.10 under the substantive reading), and the ordering of the hazard rates on a neighbourhood of the origin are not affected.
\end{itemize}
...
\paragraph{Minor errata and a remark in \cite{ShojaeeAsadiFinkelstein2022}.} The exact-arithmetic audit described in Section~\ref{sec:repro} checked the remaining finitely testable claims of both manuscripts; three items in \cite{ShojaeeAsadiFinkelstein2022} are worth recording here, none of which contradicts a main claim.

(i) \emph{Theorem~4.1(b).} In the multiplicative model $r_i(t)=\lambda_i r(t)$ with $\lambda_1\le\lambda_2$, $r(t)\to\infty$, $\alpha_i>0$, that statement asserts $r(t,\bar\alpha)-(\alpha_1/\bar\alpha)r_1(t)\to0$ as $t\to\infty$ if and only if $r(t)\exp\bigl(-(\alpha_2\lambda_2-\alpha_1\lambda_1)\int_0^t r(u)\,du\bigr)\to0$, under the hypothesis $\alpha_1\le c\alpha_2$ where $c=\lambda_2/\lambda_1$. The ``only if'' direction fails on the boundary $\alpha_1\lambda_1=\alpha_2\lambda_2$, which the hypothesis permits: there $\sum_i p_i\bar F_i^{\alpha_i}(t)=e^{-\alpha_1\lambda_1\int_0^tr}$ identically, hence $r(t,\bar\alpha)\equiv(\alpha_1/\bar\alpha)r_1(t)$ and the difference is identically $0$, while the displayed condition reduces to $r(t)\to0$, incompatible with the standing assumption $r(t)\to\infty$. (The simplest instance is $\lambda_1=\lambda_2$ and $\alpha_1=\alpha_2$.) Replacing the hypothesis by the strict inequality $\alpha_1<c\alpha_2$, i.e.\ $\alpha_1\lambda_1<\alpha_2\lambda_2$, repairs the biconditional.

(ii) \emph{Example~3.6.} For $\bar F_1(t)=e^{-0.8t}$, $\bar F_2(t)=e^{-0.4t}$, $(p_1,p_2)=(0.7,0.3)$ and $(\alpha_1,\alpha_2)=(2,5)$, the hazard rate printed there,
\[
r(t,\bar\alpha)=\frac1{2.9}\cdot\frac{1.12\,e^{-1.6t}+0.6\,e^{-2t}}{e^{-1.6t}+e^{-2t}},
\]
omits the weights $p_i$ in the denominator, which should read $0.7e^{-1.6t}+0.3e^{-2t}$: the printed formula gives $r(0,\bar\alpha)=43/145\approx0.297$ while the correct value is $86/145\approx0.593$. The conclusion drawn in the example ($r(t,\bar\alpha)<r_1(t)=0.8$) is unaffected.

(iii) \emph{Theorem~5.2, condition~(i), for $\alpha_i<0$.} The printed hypothesis that $(\alpha_i/\bar\alpha)r_{F_i}(t)$ be increasing in $i$ is, since $\bar\alpha<0$, equivalent to $\alpha_ir_{F_i}(t)$ \emph{decreasing} in $i$; a reader who replaces it by ``$\alpha_ir_{F_i}$ increasing'' will construct apparent counterexamples to the claim. Under the correct reading the claim held on all $5741$ admissible instances tested in the audit (a bounded exact-rational search, not a proof). The statement would benefit from spelling out the $\alpha_i<0$ case.
...
\paragraph{The $\mathcal W_n$, $\alpha\le0$ halves.} The instances (1) to (4) all lie in $\mathcal V_n$ with $\alpha>0$. The complementary halves of the same statements (the class $\mathcal W_n$ with $\alpha\le0$) fail as well, on the following two instances, both certified in exact rational arithmetic by the audit scripts of Section~\ref{sec:repro}.

(5) $n=3$; $p=(\tfrac{17}{44},\tfrac5{11},\tfrac7{44})$ and $\gamma=(7,9,2)$ are comonotone, so $[\begin{smallmatrix}p\\ \gamma\end{smallmatrix}]\in\mathcal W_3$; $M_T=\tfrac{17}{20}I+\tfrac3{20}\Pi_{13}$ gives $q=(\tfrac{31}{88},\tfrac5{11},\tfrac{17}{88})$ and $\delta=(\tfrac{25}{4},9,\tfrac{11}{4})$; $\bar G(t)=e^{-t}$, $\theta=1$, $\alpha=-2$. By \eqref{eq:reduction} below, $h_{U_3}-h_{V_3}=\tilde h_{p,\gamma}(y)-\tilde h_{q,\delta}(y)$ with $y=e^{2t}>1$. Writing $w=e^{-t/2}$, the difference is the rational function
\begin{equation}\label{eq:whalf}
h_{U_3}(t)-h_{V_3}(t)=-\frac{w^{8}B(w)}{D(w)},
\end{equation}
\begin{align*}
B(w)&=5440-6820w^{3}-1581w^{11}-8500w^{17}+7840w^{20}-4913w^{25}+3689w^{31}+357w^{45},\\
D(w)&=3200+2720w^{8}+2480w^{11}+2108w^{19}+1360w^{25}+1120w^{28}+1156w^{33}+868w^{39}+476w^{53}>0.
\end{align*}
Now $B(0)=5440>0$ and $B(1)=-4488<0$, and $B$ has exactly one root $w_*$ in $(0,1)$ (machine-certified, in $(\tfrac78,\tfrac9{10})$ since $B(\tfrac78)>0>B(\tfrac9{10})$). Hence $h_{U_3}-h_{V_3}$ is positive for $t<-2\log w_*$ and negative beyond it: the claimed $U_3\le_{\hr}V_3$ fails, the hazard rates cross exactly once, and the $\mathcal W_n$, $\alpha\le0$ half of Statement~\ref{st:SKF} is false. Exact witnesses: the difference equals $-161379121858494821/28929682881644134876<0$ at $e^{-t}=\tfrac14$ and is positive ($\approx0.16358$, an exact algebraic value) at $e^{-t}=\tfrac78$. The same single-transform instance contradicts the $\mathcal W_n$ half of Corollary~3.2 (the case $k=1$).

For Theorem~3.9 a single $T$-transform does not suffice: that statement requires a product of $k\ge2$ transforms with different structures and intermediate matrices in the class. A certified two-step chain is

(6) $p=(\tfrac7{26},\tfrac4{13},\tfrac{11}{26})$, $\gamma=(3,8,9)$, so $[\begin{smallmatrix}p\\ \gamma\end{smallmatrix}]\in\mathcal W_3$; $M_{T_1}=\tfrac1{20}I+\tfrac{19}{20}\Pi_{23}$ gives $[\begin{smallmatrix}q_1\\ \gamma_1\end{smallmatrix}]=[\begin{smallmatrix}7/26&217/520&163/520\\ 3&179/20&161/20\end{smallmatrix}]\in\mathcal W_3$; then $M_{T_2}=\tfrac14I+\tfrac34\Pi_{13}$ gives $q=(\tfrac{629}{2080},\tfrac{217}{520},\tfrac{583}{2080})$ and $\delta=(\tfrac{543}{80},\tfrac{179}{20},\tfrac{341}{80})$; $\bar G(t)=e^{-t}$, $\theta=1$, $\alpha=-1$. In $w=e^{-t/80}$ the difference $h_{U_3}-h_{V_3}$ is a rational function whose numerator has degree $855$; its exact rational values are $\tfrac{7881}{83200}>0$ at $w=1$ (i.e.\ $t=0$), negative ($\approx-0.02834$) at $w=\tfrac{49}{50}$, and $\tfrac1{20}>0$ at $w=\tfrac12$. The claimed order therefore fails on an interior band around $t\approx1.6$, and the signs $+,-,+$ force at least two crossings: the $\mathcal W_n$, $\alpha\le0$ half of Theorem~3.9 is false as well.

\paragraph{Theorem~3.4 fails for $\alpha>0$.} Theorem~3.4 of \cite{SahooKayalFinkelstein2026} (page~12) is a usual-stochastic-order statement in the scale parameters: if $t^2g(t)$ is increasing in $t>0$, then for $\alpha<\gamma_1$ with $p\in\varepsilon_n^+$ (or $\mathcal D_n$), $\theta,\xi\in\mathcal D_n^+$ (or $\varepsilon_n^+$) and $\theta\prec^w\xi$ (the increasing partial sums of $\theta$ dominate those of $\xi$), one has $U_n(p,\gamma,\theta)\ge_{\st}V_n(p,\gamma,\xi)$. Take $n=2$, scalar $\gamma=3$, $\alpha=\tfrac23<\gamma$, $p=(\tfrac13,\tfrac23)\in\varepsilon_2^+$, and $\theta=(3,1)$, $\xi=(\tfrac72,\tfrac12)\in\mathcal D_2^+$; the weak supermajorization holds since $1\ge\tfrac12$ and $4=4$. For the Lomax baseline $\bar G(t)=1/(1+t)$, for which $t^2g(t)=t^2/(1+t)^2$ is strictly increasing in $t>0$, the sign of $\bar G_{U_2}-\bar G_{V_2}$ equals the sign of the inner difference
\begin{equation}\label{eq:thm34}
d(t)=\sum_i p_i\Bigl[\Bigl(\frac{\theta_i}{\theta_i+t}\Bigr)^{2}-\Bigl(\frac{\xi_i}{\xi_i+t}\Bigr)^{2}\Bigr]
=\frac{t\bigl(1722+4565t+3664t^{2}+1053t^{3}+4t^{4}-28t^{5}\bigr)}{3(t+1)^{2}(t+3)^{2}(2t+1)^{2}(2t+7)^{2}},
\end{equation}
since $\alpha\gamma=2$ and $\alpha>0$. The quintic factor has a single sign change in its coefficient sequence, so exactly one positive root by Descartes' rule, lying in $(7,8)$ because the factor equals $113400$ at $t=7$ and $-89246$ at $t=8$. Hence $d(t)>0$ on $(0,t_*)$ and $d(t)<0$ on $(t_*,\infty)$; exact witnesses are $d(1)=305/3888>0$ and $d(20)=-1579547560/2598836427243<0$. The claimed $U_2\ge_{\st}V_2$ therefore fails for large $t$, and since the difference is positive near the origin the reverse $V_2\ge_{\st}U_2$ fails too: the two mixtures are stochastically incomparable. (A three-component instance is certified by the same script: $p=(\tfrac{25}{86},\tfrac{27}{86},\tfrac{17}{43})$, $\gamma=6$, $\alpha=\tfrac56$, $\theta=(8,8,5)$, $\xi=(9,8,4)$.) The failure is located in the Schur-concavity step of the published proof (its equation (3.7)), where factors whose signs compete, since $\bar G^{\alpha\gamma-1}(t/\theta)$ and $(t/\theta)^2g(t/\theta)/\theta^2$ move in opposite directions for $\theta_i>\theta_j$ depending on the sign of $\alpha\gamma-1$, are combined as if the displayed sign followed from the hypotheses. The audit's sampled violations occur on both sides of $\alpha\gamma=1$, so an $\alpha\gamma\le1$ restriction does not repair the statement; the $\alpha<0$ half held on all $589$ admissible sampled instances (bounded search, not a proof). The proof of Theorem~3.6 there invokes Theorem~3.4 only in that $\alpha\le0$ regime, so the counterexample does not propagate to it.

\paragraph{Two invalid counterexamples printed in the manuscript.} Counterexamples~3.7 and~3.8 of \cite{SahooKayalFinkelstein2026} (page~16) are offered to show the need for the hypotheses of Theorem~3.6; their numerical content is wrong in both cases. Counterexample~3.7 prints $\bar G_{U_3}(20)-\bar G_{V_3}(20)=-0.08336543$ for $p=(0.05,0.3,0.65)$, $q=(0.7,0.26,0.4)$, $\gamma=2$, $\theta=(5,3.5,2)$, $\xi=(5,3,1.5)$, $\alpha=-0.3$ and the Pareto baseline $\bar G(t)=t^{-1/2}$; recomputation at $60$-digit precision gives $+0.06744747$: the difference is positive at $t=20$, in the direction claimed by Theorem~3.6, and the printed number is simply erroneous. Counterexample~3.8 reports $h_{U_3}-h_{V_3}$ taking the values $-1.110223\times10^{-16}$ and $+2.220446\times10^{-16}$ at $t=6$ and $t=7$; but for the Pareto baseline with scalar $\gamma$ the hazard rate displayed there simplifies to $h_{U_3}(t)=\gamma/(2t)=h_{V_3}(t)$ identically, so the two hazard rates are equal and the reported values are floating-point roundoff.
...
\]
which equals $\tfrac{11}{25}-\tfrac{8}{19}=\tfrac{9}{475}>0$ at $t=\log2$, contradicting $r_{U_3}\le r_{V_3}$; so Theorem~3.11 and Corollary~3.3 fail under this reading too. Under this reading, however, the failure is not specific to $n\ge3$. Theorems~3.7 and~3.10 of \cite{SahooKayalFinkelstein2026} would give $U_2\ge_{\hr}V_2$ and $U_2\le_{\mathrm{rh}}V_2$, hence $U_2\ge_{\st}V_2$ and $U_2\le_{\st}V_2$, so $U_2$ and $V_2$ would have the same distribution; this is false for the pair $(3,5)$, $(4,4)$ with equal weights, for which $S_{(3,5)}-S_{(4,4)}=\tfrac12q^3(1-q)^2>0$ and, directly, $r_{(3,5)}(\log2)=\tfrac{17}{59}>\tfrac{4}{15}=r_{(4,4)}(\log2)$. Since Theorem~3.7 holds for this pair (Remark~\ref{rem:twocomp}), Theorem~3.10 cannot hold under reading (b). The same argument shows that, under reading (b), Theorems~3.8 and~3.11 cannot both hold for any admissible pair for which $U_n$ and $V_n$ are not equal in distribution. The failure under this reading is generic rather than exceptional: in the audit's exact-arithmetic bounded search over admissible two-component instances of the $\mathcal V_2$, $\alpha>0$ branch (exponential baseline, $\theta=1$, $T$-transformed pairs), $2279$ of $2500$ violate the literal reversed-hazard conclusion of Theorem~3.10.

We therefore regard (a) as the substantive reading: under it, the two-component Theorem~3.10 reduces to a two-component hazard rate comparison, which the audit confirmed on all admissible sampled instances ($2\times3000$ tested instances across the two branches in the shipped default run; a bounded-search confirmation, not a proof), while Theorem~3.11, Corollary~3.3 and Theorem~3.12 fail for $n\ge3$ at exactly the instances that refute Theorem~3.8, Corollary~3.2 and Theorem~3.9: instances (1) to (4) for the $\mathcal V_n$, $\alpha\ge0$ halves and instances (5) and (6) for the $\mathcal W_n$, $\alpha\le0$ halves.
...
\paragraph{The audit layer.} The claims added in Section~\ref{sec:relation} (the $\mathcal W_n$, $\alpha\le0$ instances (5) and (6), the Theorem~3.4 counterexample, the two invalid printed counterexamples, the bounded-search statistics, and the \cite{ShojaeeAsadiFinkelstein2022} errata) come from a separate audit of all finitely testable ordering claims in the two manuscripts, recorded in \texttt{memo.md} in the directory \texttt{mixture-audit} of the accompanying research repository and implemented by scripts that share the same exact-arithmetic discipline (exact rationals, Sturm counts and isolating intervals through SymPy; $60$-digit evaluation where genuinely algebraic quantities are compared):
\begin{list}{}{\setlength{\leftmargin}{1em}\setlength{\itemsep}{2pt}\small}
\item \texttt{memo.md}. The audit record: per-claim status, parameter tuples, witnesses and sample counts.\\
SHA-256 \texttt{7af6759e843e22ec2cb558a0460ac90406211c5cb0ef89d02e5f066139b775ee}
\item \texttt{certify\_thm34.py}. The instance of the Theorem~3.4 paragraph: its hypotheses, the identity \eqref{eq:thm34}, the root counts and the witnesses $305/3888$ and $-1579547560/2598836427243$, and the three-component instance mentioned there.\\
SHA-256 \texttt{9bde7bca907e7ce679ac49232a5945dfd855db8d6fda982db4e8f7677d9fe6b0}
\item \texttt{certify\_whalf.py}. Instances (5) and (6): the $\mathcal W_3$ membership tests including the intermediate matrix of (6), the $T$-transforms, the identity \eqref{eq:whalf} and the signed witnesses.\\
SHA-256 \texttt{7734bae9c52d33053f82689252edad8b36ace72f4619f77cbad59facaaf3dd3b}
\item \texttt{verify\_paper\_cex.py}. Rechecks the printed examples and counterexamples of both manuscripts, including the two invalid ones discussed above.\\
SHA-256 \texttt{01812f258d539495decaa032abe4d614a43a764bf9de6a389cbd1cb1baf104e8}
\item \texttt{audit\_skf\_st.py}, \texttt{audit\_skf\_hr.py}, \texttt{audit\_saf.py}, with shared helpers \texttt{audit\_lib.py}. The bounded searches behind the sample counts quoted above (stochastic order, hazard and reversed hazard claims of \cite{SahooKayalFinkelstein2026}, and the tested claims of \cite{ShojaeeAsadiFinkelstein2022}); hypotheses are enforced exactly (the $\mathcal V_n$/$\mathcal W_n$ and $\mathcal U_n$ membership tests, majorization, weak sub- and supermajorization, and the $T$-transform factorizations).
\end{list}
Sample counts quoted in Section~\ref{sec:relation} are bounded-search statistics, not proofs.
