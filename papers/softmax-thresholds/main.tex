...
\author{Benjamin Guo\\ Hraness (\url{hraness.com})\thanks{Disclosure: the manuscript and its proofs were prepared with AI research agents and were independently machine-checked (exact rational root enclosures, symbolic verification of every identity used in the proofs, and exact rational evaluation of every counterexample); see Section~\ref{sec:certified}.}}
...
\smallskip\noindent\textbf{2020 Mathematics Subject Classification.} 26B25, 26D15, 26E60; secondary 68T05, 91G05.
\end{abstract}
...
A set $D\subseteq\R^n$ is symmetric if it is invariant under permutations of the coordinates, and a function $f\colon D\to\R$ on a symmetric set is symmetric if $f(x\Pi)=f(x)$ for every $x\in D$ and every permutation matrix $\Pi$. A function $f\colon D\to\R$ on a symmetric set is \emph{Schur-convex on $D$} if $x,y\in D$ and $x\maj y$ imply $f(x)\ge f(y)$, and \emph{strictly Schur-convex on $D$} if in addition $f(x)>f(y)$ whenever $y$ is not a permutation of $x$. It is (strictly) Schur-concave if $-f$ is (strictly) Schur-convex.
...
\begin{proof}
Let $v\in D$ and $i\ne j$ with $v_i\le v_j$, and write $c=(v_i+v_j)/2$, $r=(v_j-v_i)/2\ge 0$. For $\rho\in[0,r]$ let $v(\rho)$ be the vector obtained from $v$ by replacing $(v_i,v_j)$ with $(c-\rho,c+\rho)$. If $r>0$ then $v(\rho)=\frac{r+\rho}{2r}\,v+\frac{r-\rho}{2r}\,vQ$, where $Q$ exchanges the coordinates $i,j$ (the two coefficients are nonnegative with sum $1$; equivalently $v(\rho)=\frac{\rho}{r}v+(1-\frac{\rho}{r})v(0)$ with $v(0)=\frac12(v+vQ)$); since $D$ is symmetric and convex, $v(\rho)\in D$. Let $T=\lambda I+(1-\lambda)Q$. Since $vT=\lambda v+(1-\lambda)vQ$, comparing coefficients gives $vT=v(\rho)$ with $\rho=(2\lambda-1)r$ when $\lambda\ge\frac12$, and $vT=v(\rho)Q$ with $\rho=(1-2\lambda)r$ when $\lambda<\frac12$. In both cases $vT\in D$. Put $\varphi(\rho)=f(v(\rho))$. Then $\varphi'(\rho)=\partial_j f(v(\rho))-\partial_i f(v(\rho))>0$ for $\rho\in(0,r]$ by \eqref{eq:segment-hyp}, because $v(\rho)\in D$ has $v(\rho)_i=c-\rho<c+\rho=v(\rho)_j$. Hence $\varphi$ is strictly increasing on $[0,r]$, and by the symmetry of $f$,
\[
...
\begin{gather*}
F(u)=B_{-1}(u)=\frac{\sum_k u_k e^{-u_k}}{\Sigma(u)},\qquad \Sigma(u)=\sum_k e^{-u_k};\\
...
\item[(d)] $\displaystyle \partial_i B_\tau(x)=\frac{e^{\tau x_i}\bigl(1+\tau(x_i-B_\tau(x))\bigr)}{Z_\tau(x)}$; in particular $\partial_i F(u)=\dfrac{e^{-u_i}(1-u_i+F(u))}{\Sigma(u)}$ and $\partial_i H(u)=\dfrac{e^{u_i}(1+u_i-H(u))}{Z(u)}$;
...
\Sigma(u)^2\bigl(\partial_1F(u)-\partial_2F(u)\bigr)=\sum_{k=1}^n e^{-u_k}\psi(u_k)=P+\sum_{k=3}^n e^{-u_k}\psi(u_k).
\end{equation}
Consequently, with $N=N(x,y)=P+m\,\psi(0)$,
\begin{equation}\label{eq:pair-bound}
\Sigma(u)^2\bigl(\partial_1F(u)-\partial_2F(u)\bigr)\ \ge\ \min\{P,N\},
\end{equation}
and equality $\Sigma(u)^2(\partial_1F(u)-\partial_2F(u))=N$ holds when $u_3=\dots=u_n=0$.
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:basic}(d), writing $\Sigma=\Sigma(u)$, $\Sigma\,\partial_iF=e^{-u_i}(1-u_i+F)$ and $\Sigma F=\sum_k u_ke^{-u_k}$, hence
\[
\Sigma^2\partial_iF=e^{-u_i}\Bigl[(1-u_i)\Sigma+\sum_k u_ke^{-u_k}\Bigr]=e^{-u_i}\sum_k(1-u_i+u_k)e^{-u_k}.
\]
...
\begin{aligned}
J(c,z)&:=F(c-z,c+z,0,\dots,0)-F(c,c,0,\dots,0)\\
...
\frac{mc\tanh(z/2)}{z}-(m+2q)\ \ge\ \frac{3mc\,\eta}{8}>0,\qquad\text{hence}\qquad J(c,z)>0 .
\end{equation}
\end{lemma}
...
\]
using $z^2\le 3\eta$. The positivity of $J(c,z)$ then follows from \eqref{eq:split}, all other factors there being positive.
\end{proof}
...
\[
F(u)-F(u^\circ)=F(C-z,C+z,0,\dots,0)-F(C,C,0,\dots,0)=J(C,z)>0
\]
...
The threshold is decided by the configuration in which all but two coordinates sit at the lower end of the range: by Lemma~\ref{lem:pair} the pair derivative is smallest there, and Lemma~\ref{lem:split} shows that at that configuration a small symmetric split of the pair around $c$ increases $F$ exactly when $g(c)<0$, that is, when $c>c_n$. Expanding \eqref{eq:split} in $z$ gives $J(c,z)=\dfrac{q\,(m(c-2)-4q)}{(m+2q)^2}\,z^2+O(z^4)=\dfrac{-q\,g(c)}{(m+2q)^2}\,z^2+O(z^4)$. The equation $g(c_n)=0$ is thus the exact boundary, and the construction above only makes the choice of $z$ explicit.
\end{remark}
...
For $X\in(0,\infty)^n$ and $p\in\R$ the Lehmer mean is $\mathcal{L}_p(X)=\sum_k X_k^{p}/\sum_k X_k^{p-1}$ \cite{Lehmer1971,Bullen2003}. For $x\in\R^n$, $\tau\in\R$ and $\varepsilon>0$, with $X=(e^{\varepsilon x_1},\dots,e^{\varepsilon x_n})$ and $p=\tau/\varepsilon$,
\begin{equation}\label{eq:lehmer}
\mathcal{L}_{p}(X)=\frac{Z_\tau(x)}{Z_{\tau-\varepsilon}(x)},\qquad
\frac{1}{\varepsilon}\log \mathcal{L}_{p}(X)=\frac{\log Z_\tau(x)-\log Z_{\tau-\varepsilon}(x)}{\varepsilon}\ \xrightarrow[\varepsilon\to 0^+]{}\ \frac{\partial}{\partial\tau}\log Z_\tau(x)=B_\tau(x).
\end{equation}
Schur-geometric convexity of $\mathcal{L}_p$ (monotonicity for the majorization of $\log X$) therefore transfers to Schur-convexity of $B_\tau$ in the limit, without strictness. Two known results transfer in this way.

(1) Fu, Wang and Shi \cite[Theorem~1.4(II)]{FuWangShi2016} prove that for $p>\frac12$ and every $A>0$ the mean $\mathcal{L}_p$ is Schur-geometrically convex on the box $\bigl[\bigl(\tfrac{p-1}{p}\bigr)^2A,\,A\bigr]^n$. Let $\tau>0$, $x\maj y$ in $[a,b]^n$ and $0<\varepsilon<\tau$. Then $\log X=\varepsilon x$, $\log Y=\varepsilon y$, $\varepsilon x\maj\varepsilon y$, and $X,Y$ lie in the box with $A=e^{\varepsilon b}$ provided $e^{\varepsilon(b-a)}\le\bigl(\tfrac{\tau}{\tau-\varepsilon}\bigr)^2$, that is, $\varepsilon(b-a)\le-2\log(1-\varepsilon/\tau)$. Since $-\log(1-w)\ge w$ for $0\le w<1$, this holds for every $\varepsilon\in(0,\tau)$ as soon as $\tau(b-a)\le 2$. Then $\mathcal{L}_p(X)\ge \mathcal{L}_p(Y)$ for all small $\varepsilon$, and \eqref{eq:lehmer} gives $B_\tau(x)\ge B_\tau(y)$. This recovers the nonstrict form of Corollary~\ref{cor:dimfree} for $\tau>0$ (and, by reflection, for $\tau<0$). The bounded-ratio condition of that theorem transfers to exactly the constant $2$; it does not give $c_n$, the strictness at the threshold, or the violations.

(2) For $n=2$ the Lehmer mean $\mathcal{L}_p=G(p,p-1)$ is a Gini mean, and the Schur-geometric convexity of the two-variable Gini means $G(r,s;x,y)$ for $r,s\ge 0$ (Shi, Jiang and Jiang \cite{ShiJiangJiang2009}; see also Witkowski \cite{Witkowski2011} for a four-parameter family containing them) holds on all of $(0,\infty)^2$, so \eqref{eq:lehmer} gives the nonstrict form of Proposition~\ref{prop:n2}. The direct proof above is shorter and gives strictness.
\end{remark}
...
\begin{proof}
Let $V=\sum_{k\ne i,j}w_ke^{w_k}$. Then $H(w)=(V+2\varepsilon e^{\varepsilon})/(E+2e^{\varepsilon})$ and $H(w')=\bigl(V+2e^{\varepsilon}(\varepsilon\cosh z+z\sinh z)\bigr)/(E+2e^{\varepsilon}\cosh z)$. Hence
\[
(E+2e^{\varepsilon}\cosh z)\bigl(H(w')-H(w)\bigr)=\bigl[V-H(w)E\bigr]+2e^{\varepsilon}\bigl[\varepsilon\cosh z+z\sinh z-H(w)\cosh z\bigr],
\]
and $V-H(w)E=H(w)(E+2e^{\varepsilon})-2\varepsilon e^{\varepsilon}-H(w)E=2e^{\varepsilon}(H(w)-\varepsilon)$. Collecting terms, the right side equals $2e^{\varepsilon}\bigl[(\varepsilon-H(w))(\cosh z-1)+z\sinh z\bigr]$, and $z\sinh z/(\cosh z-1)=z\coth(z/2)$.
\end{proof}
...
\begin{remark}
For small $n$ the negative side is the more tolerant one on the simplex ($2c_n>d_n$), for $n\ge 58$ the positive side is. Both thresholds are at least the box threshold $c_n$, with equality only in the case $d_3=c_3$, as they must be: $\Simp_S$ contains points with entries anywhere in $[0,S]$, but the fixed total prevents more than one large entry when $\tau>0$ (Lemma~\ref{lem:convexbound}) and, when $\tau<0$, keeps the midpoint $c$ of any pair at most $K/2$.
\end{remark}

\section{A counterexample to an unrestricted claim}\label{sec:counterexample}

\begin{sloppypar}
Amir, Bettini and Prorok \cite{AmirBettiniProrok2026} analyse the heterogeneity gain of cooperative multi-agent systems in terms of the Schur-convexity or Schur-concavity of the aggregators that combine the efforts of the agents into task scores and the task scores into a global reward. We quote from the arXiv version v4 of the paper (1 March 2026, 32 pages, SHA-256 \texttt{40003616 22a21d9f 829d1a77 33f54a47 7345ecea 90668c53 ea410151 bd40b3fe}).\footnote{We also inspected a 32-page file marked ``Published as a conference paper at ICLR 2026'' (SHA-256 \texttt{4473fb91 3f6cb4a6 4f6db5fa 92ae3532 0598cc89 d029f930 c88e747c 84ec0d2f}); its text agrees with v4 in every passage quoted here. We could not re-locate a public URL for that file, and we could not retrieve the OpenReview record of the paper (the site answered our requests with an access challenge), so we cite neither by page and cannot confirm that the arXiv text is identical to the official camera-ready version.} Table~3 of v4 (Appendix~I, page~21) lists parametric families of aggregators together with their Schur properties.
\end{sloppypar}
The row for the softmax aggregator defines
\[
\mathrm{Softmax}_t(x)=\sum_{i=1}^{N}\frac{e^{tx_i}}{\sum_{j=1}^{N}e^{tx_j}}\,x_i,\qquad t\in\R,
\]
which is $B_t(x)$ in our notation, and states: ``Strictly Schur-convex for $t>0$. Strictly Schur-concave for $t<0$.'' The caption of the table specifies that the Schur properties are meant ``on nonnegative inputs''. The introduction (page~2) summarizes the row as ``the soft-max operator switches from Schur-concave to Schur-convex as its temperature increases''. No restriction on the size of $t$ relative to the inputs is stated. The first arXiv version v1 (11 June 2025, 25 pages, SHA-256 \texttt{14c3a02a b1427008 beb897cf 544fe703 fd642622 c79b6778 4a5291b7 e7b7fa82}) carries the same caption clause and the same row (Table~3, page~20).
...
The witnesses are consistent with Theorems~\ref{thm:box} and~\ref{thm:simplex}: since $e^{3/5}<2$ (certified in Section~\ref{sec:certified}), $\tau=8\log 2>\tfrac{24}{5}>2c_3>d_3=c_3$, so in (a) and (b) the product $|\tau|(b-a)=\tau$ exceeds $c_3$, in (c) $\tau S=\tau$ exceeds $d_3$, and in (d) $|\tau|S=\tau$ exceeds $2c_3$. Witnesses with rational coordinates exist for every $n\ge 3$ and every temperature above the thresholds (on $\Simp_S$, for rational $S$): the strict inequalities of Theorems~\ref{thm:box}(ii) and~\ref{thm:simplex} are open conditions on the coordinates of the explicit witnesses, whose second vector is the $T$-transform with $\lambda=\frac12$ of the first, so the first vector may be moved to a nearby vector with rational coordinates (and the same total) and the second vector recomputed. The values of $B_\tau$ at such witnesses are in general not rational; the rational values in Proposition~\ref{prop:witness} come from the choice $\tau x_i\in(\log 2)\mathbb{Z}$.
...
Thus the qualifier ``on nonnegative inputs'' does not rescue the row; what is true is the bounded-range statement of Corollary~\ref{cor:nonneg}. The proof of Theorem~3.4 of \cite{AmirBettiniProrok2026} (Appendix~G.4, page~19 of v4) begins with ``When $t\le 0$, $T_j$ is Schur--concave, so $\Delta R=0$ by Thm.~3.2'', which invokes the unrestricted statement, and the same proof invokes it for the outer aggregator: ``$U$ is Schur--concave for $\tau\le 0$, and Schur--convex for $\tau\ge 0$, hence it is maximized by the uniform distribution in the former case, and by a 1-hot vector in the latter case''. The caption of Figure~5(a) of v4 (page~9) reads ``HetGPS with Softmax aggregators ($\tau\in\mathbb{R}$). $\tau\ge 0$ is Schur-convex; $\tau\le 0$ is Schur-concave.'' We do not examine here which conclusions of that paper survive under the bounded-range statement.

\section{Certified computations}\label{sec:certified}

All numerical claims of this paper are of three kinds: the enclosures of the constants $c_n$, $d_n$ in Table~\ref{tab:constants} (used in Proposition~\ref{prop:crossover}), the rational values of Proposition~\ref{prop:witness}, and the inequality $e^{3/5}<2$. They are established by exact rational arithmetic, independently of any floating-point evaluation. In addition, every algebraic identity used in the proofs was verified symbolically, and the explicit constructions of Theorems~\ref{thm:box}(ii) and~\ref{thm:simplex} were evaluated in high-precision arithmetic as a consistency check. The scripts are in the directory \texttt{papers/softmax-thresholds/verify/} of the repository \url{https://github.com/hraness/algal-lab}; they use only the Python standard library (\texttt{fractions}), SymPy and mpmath, and import nothing from any other code base.

\subsection{Exact enclosure of the exponential}
For a rational $x\in[0,8]$ put $y=x/8\in[0,1]$ and $p_{30}(y)=\sum_{k=0}^{30}y^k/k!$. For $k\ge 31$ the ratio of consecutive Taylor terms $y^{k+1}/(k+1)!$ and $y^k/k!$ is $y/(k+1)\le y/32$, so the remainder is bounded by a geometric series and
\[
p_{30}(y)\ \le\ e^{y}\ \le\ p_{30}(y)+\frac{y^{31}/31!}{1-y/32}=:U(y),
\qquad\text{hence}\qquad p_{30}(y)^8\ \le\ e^{x}\ \le\ U(y)^8 .
\]
Both bounds are rational numbers computed exactly; the width $U(y)^8-p_{30}(y)^8$ is below $10^{-29}$ on $[0,8]$. For $x=\tfrac35$ the upper bound is below $2$, which certifies $e^{3/5}<2$, that is, $\log 2>\tfrac35$ and $8\log 2>\tfrac{24}{5}$.
...
The Schur properties of two-variable Gini means $G(r,s;x,y)$ with respect to $(x,y)$ were determined by Shi, Jiang and Jiang \cite{ShiJiangJiang2009}, and Witkowski \cite{Witkowski2011} treated a four-parameter family containing the Gini and Stolarsky means, proving Schur-geometric convexity in $(x,y)$ under a sign condition on the parameters. Lehmer's paper \cite{Lehmer1971} is the source of the means $\mathcal{L}_p$; the handbook of Bullen \cite{Bullen2003} surveys them. For $n$ variables, Fu, Wang and Shi \cite{FuWangShi2016} proved Schur-convexity, Schur-geometric convexity and Schur-harmonic convexity of $\mathcal{L}_p$ on boxes whose ratio of largest to smallest coordinate is bounded in terms of $p$. Remark~\ref{rem:lehmer} explains how the bounded-ratio theorem of \cite{FuWangShi2016} transfers, through $\mathcal{L}_{\tau/\varepsilon}(e^{\varepsilon x})=Z_\tau(x)/Z_{\tau-\varepsilon}(x)$, to the nonstrict dimension-free bound $\tau(b-a)\le 2$ and no further. We did not find, in these sources or in the works citing them that we inspected, a dimension-dependent bounded-range condition, a fixed-total condition, or the constants $c_n$, $d_n$. Wang and Zhang \cite{WangZhang2026} announce, in the abstract of a 2026 paper, sufficient conditions for the Schur-concavity of a set-valued extension of a generalized Bajraktarevi\'c mean. Since $B_\tau$ is itself a Bajraktarevi\'c mean, with the identity as generator and $s\mapsto e^{\tau s}$ as weight function, their conditions may apply to it; only the abstract and the publisher's preview were accessible to us, so we do not know whether the article contains a range condition, and we found no statement of the thresholds in the material available to us.

\paragraph{The Esscher premium.}
For a random variable $X$ the Esscher premium with parameter $h$ is $E[Xe^{hX}]/E[e^{hX}]$; for the uniform law on $\{x_1,\dots,x_n\}$ this is $B_h(x)$. B\"uhlmann \cite{Buhlmann1980} derived it from an economic equilibrium; Zehnwirth \cite{Zehnwirth1981} and Gerber \cite{Gerber1981} criticized it, Gerber's comment reproducing an example communicated to him by De Vylder: for the uniform law on the three points $0<z<1$, the derivative with respect to $z$ at $z=0$ of the premium is negative when $h$ is sufficiently large. That is a failure of coordinatewise monotonicity, in which the mean changes; it is not a majorization comparison, and the inspected text contains no bounded-range criterion. Van Heerwaarden, Kaas and Goovaerts \cite{vanHeerwaardenKaasGoovaerts1989} study the order preserving and order inducing properties of the Esscher principle; their abstract states that ``higher premiums might be asked for smaller risks, and also for less variable risks'', and the survey of Laeven and Goovaerts \cite{LaevenGoovaerts2008} cites the paper, together with Gerber's comment, for the fact that the Esscher premium is not monotonic. Since majorization of the entries of $x$ is exactly the convex order of the uniform empirical laws, the clause about less variable risks is the phenomenon of the negative half of Theorem~\ref{thm:box} in actuarial language, and the 1989 paper precedes us in observing that the Esscher premium can decrease under a mean-preserving spread. We could not obtain the full text of the paper, and we could not locate, in its abstract or in the works citing it that we read, an explicit statement of a range condition or of a threshold. The sharp constants $c_n$ and $d_n$, the strictness at the threshold and the crossover at $n=58$ are, to our knowledge, new; we do not claim priority for the qualitative observation over \cite{vanHeerwaardenKaasGoovaerts1989}, whose full text may contain more than its abstract announces. Goovaerts, Kaas, Laeven and Tang \cite{GoovaertsKaasLaevenTang2004} characterize additive risk measures as mixtures of exponential premiums, equivalently of Esscher premiums with a mixing function of prescribed concavity; a single fixed parameter does not satisfy their mixing conditions, and their result does not address the present thresholds.

\paragraph{Softmax operators in learning.}
Asadi and Littman \cite{AsadiLittman2017} introduced the name Boltzmann softmax operator for $B_\beta$ in the context of value iteration, showed that it is not a non-expansion and proposed the mellowmax operator (a log-sum-exp, hence a different function) instead. Polson \cite{Polson2026} shows, according to the abstract available to us (we could not retrieve the full text), that the annealed Bellman backup of Polson and Sorensen, the expectation of $Q$ under a Boltzmann kernel, is neither monotone nor a sup-norm non-expansion, that the condition $\lambda>\lambda^\star$ on the annealing parameter is substantive with $\lambda^\star$ depending on unobservable gaps between $Q$-values, and that the mellowmax certainty equivalent gives an unconditional contraction instead; the abstract contains no majorization statement, and we do not know whether the paper does. Amir, Bettini and Prorok \cite{AmirBettiniProrok2026} use Schur-convexity as the organizing principle for heterogeneity gains and list the softmax aggregator with the unrestricted Schur property discussed in Section~\ref{sec:counterexample}; their table is the only source we know of that asserts a Schur property of $B_\tau$ in dimension $n\ge 3$, and the assertion is false. The bounded-range version in Corollary~\ref{cor:nonneg} is, to our knowledge, new.
