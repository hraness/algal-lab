...
static E *HT; static int HCAP = 1<<23, hn=0;
...
int main(int argc,char**argv){
    ...
    TCAP=1<<26; TL=malloc(TCAP*sizeof*TL);
    ...
    static long ghist[420][4096]; static long qhist[420][65536];
    long triplesN0=0;
    ...
    for(int e=0;e<hn;e++){
        ...
        for(int ti=Eh[e];ti>=0;ti=tnx[ti]){
            ...
            for(int i=0;i<cn;i++){
                ...
                cnt++; shellcnt[mmv[i]]++;
                { int q=(int)llabs(c4); if(q<65535) qhist[mmv[i]][q]++; }
                { int gab=(int)gcdn_(llabs(c1),llabs(c2));
                  if(gab<4096) ghist[mmv[i]][gab]++; }
            }
            totN+=cnt;
    ...
    printf("\nper-shell |c4| profile (m: tot / q<=m/4 / q>m/2 | h(c1,c2)=1,2,3,>3):\n");
    for(int m=1;m<=K&&m<400;m++) if(shellcnt[m]){
        long lo=0,hi=0,sm=0; for(int q=1;q<65535;q++){ if(q<=m/4)lo+=qhist[m][q]; if(q>m/2)hi+=qhist[m][q]; sm+=qhist[m][q]; }
        printf("  m=%d tot=%ld q<=m/4:%ld q>m/2:%ld | h1:%ld h2:%ld h3:%ld h>3:%ld\n",
               m,sm,lo,hi,ghist[m][1],ghist[m][2],ghist[m][3],sm-ghist[m][1]-ghist[m][2]-ghist[m][3]);
    }
    printf("\n");
    return 0;
}
