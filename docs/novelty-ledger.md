...
[additive softmax boundary](softmax-additive-boundary.md),
[constrained additive flow reduction](softmax-additive-flow.md),
[negative-outer-temperature dichotomy](softmax-negative-outer.md),
...
| The smooth Boltzmann mean has an exact concave envelope and additive allocation reduces to flow | Proved least majorant `φ(Σx)`, strict at nonbinary points for `N≥2,t>0`; weighted eligible assignments with integer task capacities and linear rewards reduce exactly to flow, and every original maximizer is binary; the unrestricted equal-task case has balanced occupancies | Ratio geometry, cardinality closure, marginal-arc flow expansion and integrality are established. The gap constant already occurs in Cohen–Fausti Lemma 5.2. The exact nonlinear allocation application was not located in the inspected packet; broader priority remains unresolved |
| Divisibility decides purity at negative outer temperature | Corollary: for `M | N`, `N≥2`, `t>0`, every maximizer at every `τ≤0` is balanced pure with value `s_(N/M)` (Chebyshev plus the recorded `τ=0` and `N=M` results). Proved: for `M∤N` and `t s_q≤1`, or more generally crumb threshold below `1/⌈M/r⌉`, the spread allocation beats every pure bottleneck, so all maximizers are fractional for `τ<−2(M−1)/(eδ)`; exact witnesses `N=3,M=2` (`5/7>2/3`) and `N=5,M=3` (`5/9>1/2`); one-column reduction gives a sufficient condition for purity | Amir–Bettini–Prorok v1–v4 treat softmax only for `N=M` (Theorem 3.4, Appendix G.4 Schur-concavity); no non-square, fractional or bottleneck statement located. Chebyshev (HLP §2.17, indexed description only), balanced separable-concave optima (Gross 1956, Fox 1966, standard references not inspected) and the max-min integrality-gap mechanism are established. Their continuous Table 1 outer-`min`/inner-`max` entry implicitly assumes `N≥M`. Large-`t` indivisible regime and OpenReview forum remain open; audit memo `algal-round24-prior-art.md` SHA-256 `74e5a4736448fd0acb787b3732d5eaefd7c69f3207443f5e69aba8a89774f60e` |
...
| Exact values C(5) = 14 and C(3) = 8, C(4) = 11; C(6) ≥ 18 | C(5) = 14 proved by exhaustive canonical layer enumeration (1,905 D4 orbits, ~292 M nodes, `layer_enum5.c`; positive-control 14-set verified by two independent determinant routes and the candidate table cross-checked by brute force); C(6) ≥ 18 by verified certificates | No public value for C(5) or C(6) located beyond the repo's own certificates; enumeration completeness argued + spot-checked, LRAT corroboration in progress; lower bounds elsewhere |
| Coplanar 5-subsets of [0,n)³ count K_* n^11 + o(n^11) | Full proof draft (`research/spikes/h2-coplanar/memo.md`), adversarial review accept-with-minor; verifies brute force at n ≤ 7 | Component of the conditional `C(n) ≥ 1.03n` route; the remaining hypothesis H1 (`Z_sphere = o(n^11)`) is open per the literature gap Ω(n^10) ≤ S(n,3) ≤ O*(n^12) (Ghosal–Goenka–Keevash 2025) |
| Extended stochastic-order audit of SAF2022 and SKF2026 | Exact rational counterexamples to SKF Thm 3.4 (α > 0 branch), Thm 3.8 W_n-half, Thm 3.11 W_n-half (inheriting 3.9/3.12/Cors 3.2–3.3); SAF Thm 4.1(b) fails at the boundary `α₁λ₁ = α₂λ₂`; two SKF printed counterexamples (3.7, 3.8) are themselves invalid (wrong sign / floating-point noise); all other testable claims hold on thousands of admissible instances (`research/spikes/mixture-audit/memo.md`) | New counterexamples pending literature check; the audit is exhaustive over the two papers' finite-testable claims |
| No five points on a sphere or plane: C(17) ≥ 45, C(18) ≥ 48, C(19) ≥ 50, C(20) ≥ 53, C(21) ≥ 55, C(22) ≥ 58, C(23) ≥ 59, C(24) ≥ 62, C(25) ≥ 65, C(26) ≥ 67 | Explicit certificates in `research/extremal/claims/`, re-verified in CI against the registry snapshot, cross-checked by an independent Leibniz determinant; found by a C local search seeded with public certificates; each claim carries an unseeded control label: `under-searched` at n = 18, 20, 21 (cold starts reached 47, 52, 55, at or above the public values), `control-missing` at n = 17, 19, 22 to 26 | Beat every public value located by a same-day audit on 2026-09-24 (Demonstrandum, milesandmistakes, Numaro, DeepMind issues, Zenodo, GitHub, HuggingFace, OEIS); lower bounds only, derived from attributed public sets, the n = 21 to 26 cells uncontested (Numaro only, no coordinates) and n = 18 to 20 lightly contested on the registry's significance fields, priority not guaranteed; posted publicly on 2026-09-25 as a comment on DeepMind issue #6 (https://github.com/google-deepmind/alphaevolve_repository_of_problems/issues/6#issuecomment-5826575488), lower bounds only, no priority claim, after a fresh independent re-check (0 zero determinants, minimum |det| 2 in all ten sets) |
