...
import { boundedResearcher, evaluateGraph, laboratoryTools, mean, measure, researchManifest, scriptedResearcher, type Measurement } from "./researcher";
...
export type StudyReport = {
  contract: "algal.lab.report.v1";
  algalRevision: string;
  backend: "scripted" | "command";
  protocol: Protocol;
  instrumentDigest: string;
...
export type StudyOptions = { executor?: Executor; progress?: (message: string) => void };
type Engine = {
...
async function core(protocol: Protocol, backend: StudyReport["backend"], engine: Engine): Promise<StudyReport> {
  ...
  const frozen: { replicate: number; condition: Condition; observed: Observed[]; portfolio: Observed[]; portfolioDigest: string }[] = [];
  for (const replicate of protocol.replicateSeeds) {
    for (const condition of CONDITIONS) {
      ...
      for (let round = 0; round < protocol.rounds; round++) {
        ...
        for (let researcher = 0; researcher < protocol.researchers; researcher++) {
          ...
          const receipt = await runOrganism({
            manifest: researchManifest, args: { input: { context: json(context) } },
      ...
      const portfolioDigest = await engine.put({ contract: "algal.lab.portfolio.v1", replicate, condition, members: portfolio.map((item) => item.id) });
      frozen.push({ replicate, condition, observed, portfolio, portfolioDigest });
  ...
  return { contract: "algal.lab.report.v1", algalRevision: ALGAL_REVISION, backend, protocol, ...identities, researcherManifest, attempts, summaries };
}
...
export async function runStudy(input: unknown, directory: string, options: StudyOptions = {}): Promise<StudyReport> {
  ...
  const report = await core(protocol, options.executor ? "command" : "scripted", {
    put: (value) => store.put(value), executor: async (context) => options.executor ?? scriptedResearcher(context),
...
export async function verifyStudy(directory: string): Promise<{ ok: true; attempts: number; experiments: number; artifacts: number; reportDigest: string }> {
  ...
  if (raw.contract !== "algal.lab.report.v1" || raw.algalRevision !== ALGAL_REVISION || (raw.backend !== "scripted" && raw.backend !== "command")) throw new Error("unsupported report identity");
  ...
  const regenerated = await core(protocol, raw.backend, {
    async put(value) {
...
export function renderReport(report: StudyReport): string {
  ...
  return `# ${report.protocol.name}\n\nBackend: ${report.backend}. ${report.backend === "scripted" ? "This is a deterministic search baseline, not evidence of LLM discovery." : "Model outputs are hypotheses; measurements come from the admitted simulator."}\n\n` +
    ...
    `| Seed | Condition | Valid | Designs | Prediction MAE | Mean holdout AUC | Best holdout AUC | Coverage ≥ 0.75 |\n| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |\n` +
    report.summaries.map((s) => `| ${s.replicate} | ${s.condition} | ${s.validExperiments}/${s.attempts} | ${s.uniqueDesigns} | ${number(s.predictionMae)} | ${number(s.meanHoldoutAuc)} | ${number(s.bestHoldoutAuc)} | ${number(s.coverageAt075)} |`).join("\n") +
